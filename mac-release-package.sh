...
PACKAGE=VCP-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=VCP-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} VcashPay.exe VcashPayservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe VcashPay.exe
cp installer/lib/JavaExe.exe VcashPayservice.exe
cp -a ${FILES} nxt
